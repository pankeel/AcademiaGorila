...
public class HeaderAdapter extends RecyclerView.Adapter<HeaderViewHolder> {
    ...
    public HeaderAdapter(Context mContext, List<Post> mPostList) {
    ...
    public HeaderViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View mView = LayoutInflater.from(parent.getContext()).inflate(R.layout.rv_post_header_row_item, parent, false);
...
//        holder.price.setText(Util.currencyFormat(ad.getPrice()));
        ...
        if(post.getFeaturedMedia() != null){
            Picasso.get().load(post.getFeaturedMedia()).into(holder.imageView);
        }else{
        ...
        int dimensionInDp = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 50, displayMetrics);

        holder.itemView.getLayoutParams().width = width - dimensionInDp;
